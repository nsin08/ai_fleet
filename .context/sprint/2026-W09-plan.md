...
| Scenario persistence | Page refresh kills Pause/Reset controls | Replay controls survive navigation and refresh |
...
2. Scenario run persistence — hydrate from server on page load/navigation
3. Login page — demo credentials, session stored in localStorage
4. Live emitter scale-out — all 14 vehicles emitting in live mode
5. AI graceful degradation — no 500 if Ollama is down
6. AI grounding — inject real DB data into context before LLM call
...
**Current flow:**
```
User message → /api/ai/chat → Ollama (no data) → Hallucinated response
```
...
User message + context →
  /api/ai/chat
    ...
    → data fetcher (queries DB based on intent and entity type)
    → context builder (formats data as JSON summary for system prompt)
  → Ollama (with real data in context)
  → Grounded response with evidence references
```
...
**Current:** `activeRun` in `useState` — lost on navigation

**Target:** On mount, call `GET /api/fleet/mode` → if `mode === 'replay'` and `active_run_id` exists, call `GET /api/scenarios/runs/:runId` to rehydrate state

**Note:** The `/api/scenarios/runs/:runId` endpoint needs to be added to `scenarios.controller.ts`
...
**Acceptance Criteria**
- During replay, "On Trip" count matches the number of `on_trip` status vehicles in `vehicle_latest_state`
...
- Verify with replay running: start `scenario-mixed-alert`, confirm "On Trip" increments

---

### WP-02 Scenario Run Persistence

**User Story:**  
As an operator, if I navigate away from the Scenarios page while a replay is running, I should still be able to Pause or Reset it when I return.

**Acceptance Criteria**
- Refreshing the Scenarios page while a replay is running shows Pause/Reset buttons (not "Start Replay")
- Navigating away and back restores controls
- Dashboard replay controls also restore after navigation
- If the run ends naturally, controls correctly disappear

**Technical Tasks**
- API: Add `GET /api/scenarios/runs/:runId` endpoint in `scenarios.controller.ts`
- Web (`scenarios/page.tsx`): On mount, call `GET /api/fleet/mode`, if `active_run_id` is present, fetch that run and set `activeRun`
- Web (`page.tsx`): Same hydration logic in dashboard scenario controls
- Test: manual verify by starting replay → navigate to Alerts → navigate back → Pause button visible

---

### WP-03 Login Page (Demo Mode)
...
- Add `buildGroundedContext(intent: string, context: ChatContext): Promise<string>` function
  - Uses keyword matching to detect intent from user message
  - Fetches relevant data from DB using `getPool()` queries (not external API calls)
  - Returns a formatted string of real records to inject as a system message
- Add intent detection helper: `detectIntent(message: string): IntentKey | null`
  - Keywords → intent keys: `top driver → top_drivers`, `open alert → open_alerts`, etc.
- Modify `POST /api/ai/chat` to call `buildGroundedContext` before building the messages array
- Inject grounded context as an additional system message directly before the user message

*Frontend — `copilot-drawer.tsx`*
- Rename "Fleet Copilot" → "OpsEdge AI" in header
- Update all suggestion chip labels if they reference the old name
- Change model label display if Ollama returns model name fallback
...
|---|---|---|
| Mon Feb 23 | WP-01 (KPI) + WP-02 (scenario persistence) + WP-05 (AI degradation) | Dashboard KPI correct, scenario controls survive navigation, AI never 500s |
| Tue Feb 24 | WP-06 backend (AI grounding intent + data fetch) | `who is top driver` returns real data from DB |
| Wed Feb 25 | WP-06 frontend (rename + context injection) + WP-03 (login page) | OpsEdge AI live in drawer, login page functional |
| Thu Feb 26 | WP-04 (emitter scale-out) + WP-07 (vehicle trail) | 14 emitters active in live mode, trails visible in replay |
...
- `apps/api/src/controllers/scenarios.controller.ts`: Add `GET /api/scenarios/runs/:runId`
...
- `apps/web/src/app/page.tsx` — fix `onTripCount`, add trail state, scenario hydration
- `apps/web/src/app/scenarios/page.tsx` — scenario run hydration on mount
...
2. Add `GET /api/scenarios/runs/:runId` endpoint — needed for persistence
3. Add `detectIntent()` skeleton in `ai.controller.ts` with first 3 intent categories
4. Spike: test Ollama context injection with a hardcoded driver list and verify it stops hallucinating
...
- [ ] During replay: "On Trip" KPI >= 1 within 10 seconds of scenario start
- [ ] KPI matches number of green dots on map (manual count verified)

### WP-02 Scenario Run Persistence

- [ ] API: `GET /api/scenarios/runs/:runId` endpoint added and returns run object
- [ ] Scenarios page: hydrates `activeRun` from server on mount if `fleet/mode` has active run
- [ ] Dashboard: same hydration logic applied
- [ ] Manual verified: start replay → navigate to Alerts → navigate back → Pause button visible
