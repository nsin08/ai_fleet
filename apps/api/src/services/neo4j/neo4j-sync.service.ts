...
export class Neo4jSyncService {
  ...
  async syncAll(): Promise<void> {
    ...
    try {
      ...
      // Trigger embedding generation after successful sync — truly non-blocking.
      // Errors are caught and logged; sync callers are not affected.
      embeddingGenerator.generateAll().catch((err) =>
        log.warn('[syncAll] Embedding generation error (non-fatal)', {
  ...
  async syncDelta(): Promise<void> {
    ...
    try {
      ...
      // Trigger embedding generation after delta sync — truly non-blocking.
      // Errors are caught and logged; sync callers are not affected.
      embeddingGenerator.generateAll().catch((err) =>
        log.warn('[syncDelta] Embedding generation error (non-fatal)', {
          error: err instanceof Error ? err.message : String(err),
        }),
      );
    } catch (error) {
    ...
    try {
      ...
      for (const row of result.rows) {
        batch.push({
          ...
          // Use the PostgreSQL updated_at as epoch ms so d.updatedAt only advances
          // when the row genuinely changes, enabling correct incremental embedding.
  ...
  private async syncAlertNodes(session: any): Promise<void> {
    ...
    try {
      ...
      for (const row of result.rows) {
        await session.run(
          // Use ON CREATE SET for createdAt so it is stamped once and never
          // overwritten on re-sync. Alerts are immutable events; there is no
          // updatedAt to track, so embedding staleness relies solely on
          // a.embedding IS NULL (see embedding-generator.ts).
          `MERGE (a:Alert {id: $id})
